요청하신 채널 ID `1441159364298936340`를 기존의 채널 ID `720658789832851487`와 함께 **커스텀 보이스 채널 생성 채널 목록**에 추가했습니다.

이제 사용자가 두 채널 중 어느 곳에 입장하든, **새로운 커스텀 보이스 채널이 생성되고 자동으로 이동**될 것입니다.

아래는 수정된 최종 `index.js` 코드입니다.

## 📝 최종 `index.js` 코드 (커스텀 보이스 채널 ID 2개 반영)

`index.js` 파일에서 `CUSTOM VOICE CHAT CONFIG` 부분을 포함하여 전체 내용을 아래 코드로 교체해 주세요.
